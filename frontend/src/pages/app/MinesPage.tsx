  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      <section aria-label="Mine network summary" className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {summaryMetrics.map(([label, value, Icon, color]) => <article key={label} className="relative isolate overflow-hidden rounded-xl border border-[#21415A] bg-[#0C1A27] p-4"><div className="absolute inset-0 -z-10 bg-cover bg-center opacity-75" style={label === 'Total mines' ? { backgroundImage: `linear-gradient(90deg, rgba(12, 26, 39, 0.72), rgba(12, 26, 39, 0.3)), url(${totalMinesImg})` } : label === 'Active sites' ? { backgroundImage: `linear-gradient(90deg, rgba(12, 26, 39, 0.72), rgba(12, 26, 39, 0.3)), url(${activeSiteImg})` } : label === 'Maintenance' ? { backgroundImage: `linear-gradient(90deg, rgba(12, 26, 39, 0.72), rgba(12, 26, 39, 0.3)), url(${maintenanceImg})` } : label === 'Avg. compliance' ? { backgroundImage: `linear-gradient(90deg, rgba(12, 26, 39, 0.72), rgba(12, 26, 39, 0.3)), url(${averageComplianceImg})` } : undefined} /><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] text-[#9FB3BE]">{label}</p><p className="mt-1 text-2xl font-semibold text-[#F4F7F8]">{value}</p></div><span className={`flex h-8 w-8 items-center justify-center rounded-lg bg-[#07121C] ${color}`}><Icon className="h-4 w-4" /></span></div></article>)}
      </section>Z

...
const VisualPanel = ({ label, className = '', imageSrc }: { label: string; className?: string; imageSrc?: string }) => (
  <div className={`relative overflow-hidden border border-[#526067]/60 bg-[#151B1E] ${className}`} data-image-slot={label}>
    {imageSrc ? <img src={imageSrc} alt="Mine operations" className="absolute inset-0 h-full w-full object-cover" /> : <><div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_25%,rgba(216,138,50,0.18),transparent_32%),linear-gradient(135deg,#182226_0%,#0B0D0E_58%,#28343A_100%)]" /><div className="absolute inset-0 opacity-30 [background-image:linear-gradient(rgba(191,205,208,0.12)_1px,transparent_1px),linear-gradient(90deg,rgba(191,205,208,0.12)_1px,transparent_1px)] [background-size:42px_42px]" /></>}
    ...
    <main className="min-h-screen overflow-hidden bg-[#0B0D0E] text-[#F4F5F5] selection:bg-[#D88A32] selection:text-[#0B0D0E]">
      <nav className="fixed left-0 right-0 top-0 z-50 border-b border-[#526067]/30 bg-[#0B0D0E]/80 backdrop-blur-xl">
        <div className="mx-auto flex h-[74px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
          <Link to="/" className="group flex items-center"><img src={logoImg} alt="AgniStrot" className="h-12 w-[138px] object-contain" /></Link>
